...
ERROR()     { LOG_HANDLER_DEFAULT "$FUNCNAME" "$@" >&2; exit 1; }
CRITICAL()  { LOG_HANDLER_DEFAULT "$FUNCNAME" "$@" >&2; exit 1; }
ALERT()     { LOG_HANDLER_DEFAULT "$FUNCNAME" "$@" >&2; exit 1; }
EMERGENCY() { LOG_HANDLER_DEFAULT "$FUNCNAME" "$@" >&2; exit 1; }
